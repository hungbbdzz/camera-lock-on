...
if not exist "%GRADLE_BAT%" (
    ...
    powershell -NoProfile -ExecutionPolicy Bypass -Command "$ErrorActionPreference='Stop'; [Net.ServicePointManager]::SecurityProtocol = [Net.SecurityProtocolType]::Tls12 -bor [Net.SecurityProtocolType]::Tls13; $zip=Join-Path $env:TEMP '%GRADLE_DIST%'; Invoke-WebRequest -UseBasicParsing '%GRADLE_URL%' -OutFile $zip; if (Test-Path '%GRADLE_HOME%') { Remove-Item -Recurse -Force '%GRADLE_HOME%' }; Expand-Archive -Path $zip -DestinationPath '%GRADLE_CACHE%' -Force; Remove-Item $zip -Force"
    if errorlevel 1 (
