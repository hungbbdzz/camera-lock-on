...
A lightweight, highly configurable client-side target lock-on system for Minecraft 1.21.1 on NeoForge.

Camera Lock-On adds action-game-style targeting without changing attack reach, damage, entity AI, or server-side combat rules. Version 2.0 is a major redesign with automatic acquisition, intelligent retargeting, entity-specific aim points, a compact combat HUD, attacker awareness, and experimental sweep/AOE assistance.

## Requirements

- Minecraft 1.21.1
- NeoForge 21.1.65 or newer in the supported 21.1 range
- Client side only

The server normally does not need the mod. Camera rotation automation may be restricted by competitive servers or anti-cheat systems, so check server rules before using it in multiplayer.
...
6. Open the mod configuration from the Mods screen or assign the dedicated config keybind.
...
Camera Lock-On features 100% full UI localization out of the box for:
...
- 3D entity preview with automatic full rotation and manual drag rotation
...
- 100% translatable UI with built-in English, Chinese, Russian, Spanish, German, and Japanese
...
Smart Lock gives direct mouse input priority. Move the mouse to look away temporarily; after input stops, the camera gently returns to the target. This is the recommended mode for normal gameplay.
...
Dead Zone is optional and disabled by default. It softens small camera corrections around the center of the screen. In Smart Lock, active mouse input remains the highest-priority control layer.
...
- **Pixel Indicator** — shows one shrinking pixel box around the crosshair

The old duplicate progress pips were removed in 2.0. The shrinking box uses the selected reticle color and flashes white when acquisition completes.
...
Open **Filter → Aim Overrides** to search, add, edit, or remove saved points. The editor displays the entity model and lets the player drag the aim marker directly over it.
...
The combat HUD can be moved directly:

```text
HUD → Adjust HUD Position
```

Drag the bright card, then click outside it to apply.
...
For armor values up to 20, vanilla full and half armor sprites are shown. Above 20, the HUD uses one armor icon plus the numeric armor value.
...
In Sweep Weapons mode, Group Aim recognizes:

1. items exposing NeoForge's `SWORD_SWEEP` ability
2. items manually added by the player
...
The main settings use NeoForge's client configuration system.
...
- Searchable vanilla/modded entity selector with 3D preview
- Global and per-entity aim-point editors
- Searchable per-entity aim override manager
...
- Dedicated config keybind and 8 new unbound combat keybinds
- 100% full UI localization (i18n) with built-in English, Chinese, Russian, Spanish, German, and Japanese
...
Camera Lock-On 2.0.0 and later are distributed under **All Rights Reserved** terms. Earlier releases remain governed by the license attached to those releases.
