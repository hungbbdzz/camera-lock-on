...
public final class LockOnHudRenderer {
    ...
    private static final ResourceLocation EMPTY_HEART =
            ResourceLocation.withDefaultNamespace("hud/heart/container");
    private static final ResourceLocation FULL_HEART =
            ResourceLocation.withDefaultNamespace("hud/heart/full");
    private static final ResourceLocation HALF_HEART =
            ResourceLocation.withDefaultNamespace("hud/heart/half");
    private static final ResourceLocation BLINKING_EMPTY_HEART =
            ResourceLocation.withDefaultNamespace("hud/heart/container_blinking");
    private static final ResourceLocation BLINKING_FULL_HEART =
            ResourceLocation.withDefaultNamespace("hud/heart/full_blinking");
    private static final ResourceLocation BLINKING_HALF_HEART =
            ResourceLocation.withDefaultNamespace("hud/heart/half_blinking");

    private static final ResourceLocation HALF_ARMOR =
            ResourceLocation.withDefaultNamespace("hud/armor_half");
    private static final ResourceLocation FULL_ARMOR =
    ...
    private static void drawTargetCard(
        ...
        if (options.showHealth()) {
            if (data.maximumHealth() <= INLINE_HEALTH_LIMIT) {
                ...
                ResourceLocation container = damageFlash ? BLINKING_EMPTY_HEART : EMPTY_HEART;
                ResourceLocation full = damageFlash ? BLINKING_FULL_HEART : FULL_HEART;
    ...
    private static void drawArmor(GuiGraphics graphics, Font font, int x, int y, int armorPoints) {
        if (armorPoints <= INLINE_ARMOR_LIMIT) {
            ...
            for (int icon = 0; icon < iconCount; icon++) {
                ...
                ResourceLocation sprite = remaining >= 2 ? FULL_ARMOR : HALF_ARMOR;
    ...
    private static void drawArmorIcon(GuiGraphics graphics, int x, int y, ResourceLocation sprite) {
        graphics.blitSprite(net.minecraft.client.renderer.RenderType::guiTextured, sprite, x, y, ARMOR_SIZE, ARMOR_SIZE);
    }

    private static void drawHeart(GuiGraphics graphics, int x, int y, ResourceLocation sprite) {
        graphics.blitSprite(net.minecraft.client.renderer.RenderType::guiTextured, sprite, x, y, HEART_SIZE, HEART_SIZE);
    }
