...
public final class LockOnHudRenderer {
    ...
    private static final Identifier EMPTY_HEART =
    ...
    private static final Identifier HALF_HEART =
            Identifier.withDefaultNamespace("hud/heart/half");
    private static final Identifier BLINKING_EMPTY_HEART =
            Identifier.withDefaultNamespace("hud/heart/container_blinking");
    private static final Identifier BLINKING_FULL_HEART =
            Identifier.withDefaultNamespace("hud/heart/full_blinking");
    private static final Identifier BLINKING_HALF_HEART =
            Identifier.withDefaultNamespace("hud/heart/half_blinking");

    private static final Identifier HALF_ARMOR =
            Identifier.withDefaultNamespace("hud/armor_half");
    private static final Identifier FULL_ARMOR =
    ...
    private static void renderTargetHud(
        ...
        graphics.pose().translate(position[0], position[1]);
        graphics.pose().scale((float) options.scale(), (float) options.scale());
        drawTargetCard(
                graphics,
                minecraft.font,
                0,
    ...
    private static void drawTargetCard(
        ...
        if (options.showHealth()) {
            if (data.maximumHealth() <= INLINE_HEALTH_LIMIT) {
                ...
                Identifier container = damageFlash ? BLINKING_EMPTY_HEART : EMPTY_HEART;
                Identifier full = damageFlash ? BLINKING_FULL_HEART : FULL_HEART;
    ...
    private static void drawArmor(GuiGraphics graphics, Font font, int x, int y, int armorPoints) {
        if (armorPoints <= INLINE_ARMOR_LIMIT) {
            ...
            for (int icon = 0; icon < iconCount; icon++) {
                ...
                Identifier sprite = remaining >= 2 ? FULL_ARMOR : HALF_ARMOR;
    ...
    private static void drawArmorIcon(GuiGraphics graphics, int x, int y, Identifier sprite) {
        graphics.blitSprite(net.minecraft.client.renderer.RenderPipelines.GUI_TEXTURED, sprite, x, y, ARMOR_SIZE, ARMOR_SIZE);
    }

    private static void drawHeart(GuiGraphics graphics, int x, int y, Identifier sprite) {
        graphics.blitSprite(net.minecraft.client.renderer.RenderPipelines.GUI_TEXTURED, sprite, x, y, HEART_SIZE, HEART_SIZE);
    }
